...
public class MateriaGestion {
    ...
    public Materia buscarMateria(Materia mat) {
        ...
        if (mat.getIdMateria() != 0) {
            zz = "SELECT idMateria, nombre, año, estado FROM materia WHERE idMateria=?";
            xx = "" + mat.getIdMateria();
        } else if (!mat.getNombre().equals("")) {
            zz = "SELECT idMateria, nombre, año, estado FROM materia WHERE nombre=?";
            xx = mat.getNombre();
        }
        PreparedStatement ps;
        try {
            ps = con.prepareStatement(zz);
            ps.setString(1, xx);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
